...
select max(bs) < 70000 from (select avg(a), max(blockSize()) as bs from remote(test_cluster_two_shards, currentDatabase(), t) group by a) settings max_block_size = 5000;
...
insert into t_different_dbs select number % 1000, number % 1000 from numbers_mt(1e5);
insert into shard_1.t_different_dbs select number % 1000, number % 1000 from numbers_mt(1e5);
...
create table t4(a UInt32, b UInt32) engine=MergeTree order by a settings index_granularity = 1;
insert into t4 select number, number from numbers_mt(10);
select a from t4 group by a order by a settings max_bytes_before_external_group_by = 1, group_by_two_level_threshold = 1;
